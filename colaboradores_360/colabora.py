...
def ver_colaboradores(self, id_proyecto, editar, eliminar):
    string sql =    "SELECT"
    ...
    string auxiliar
    string id
    for (i = 0; i != R.size(); ++i)     output += "<tr><td>"
        ...
        escape(auxiliar)
        output += auxiliar + "</td><td>"
        try         if   R[i]["externo"].as<bool>() :               auxiliar = R[i]["descripcion"].as<string>()
                escape(auxiliar)
                output += auxiliar + "</td><td>"
            } else:
        ...
        } catch(  exception &e )            output += "</td><td>"
...
static PyObject* ver_colaboradores( PyObject *self, *args )  char * id_proyecto
    bool editar=False
    bool eliminar=False
    if(not PyArg_ParseTuple(args,"sbb",&id_proyecto,&editar,&eliminar)) return NULL
    return Py_BuildValue("s",ver_colaboradores(id_proyecto,editar,eliminar).c_str())


static PyMethodDef colabora_metodos[]=  {"ver_colaboradores",(PyCFunction)ver_colaboradores,METH_VARARGS},
    {NULL,NULL,0,NULL


def initcolabora(self, void):   Py_InitModule("colabora",colabora_metodos)
